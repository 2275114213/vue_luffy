<template>
	<div class="shopping-cart-wrap">
		<h3 class="shopping-cart-tit">我的购物车<small>共门{{courseCount}}课程</small></h3>
		<div class="row">
			 <el-table
			     v-loading="loading"
			    ref="multipleTable"
              ...
              <!--{{scope.row.relate_price_policy}}-->
                  ...
                  <!--有效期{{item.valid_period_text}}-->
			      	...
			      	<!--</select>-->
              <select v-model = 'scope.row.valid_period' @change = 'changeHandler(scope.row,scope.row.id,scope.row.valid_period)'>
              <option v-for = '(item,index) in scope.row.relate_price_policy' :key = 'index' :value = 'item.valid_period'>有效期{{item.valid_period_text}}</option>
            </select>
			      </template>
...
export default {
	...
	methods:{
	  changeHandler(row,courseId,newValidPeriodId,){
			...
			row.relate_price_policy.forEach((el,index)=>{
				if (el.default) {
					oldValidPeriodId = el.valid_period
				}
			})
			...
			.then(res=>{
				console.log('更新之后的',res);
				if (res.data.code ===1000) {
					this.oldValidPeriodId = res.data.valid_period;
          ...
          console.log('===============',this.shopCartList,res.data.total)
				}
			})
			.catch(err=>{
			})
     ...
     account(){
      if (this.multipleSelection.length){
        ...
        this.multipleSelection.forEach((item,index)=>{
          arr.push({
          "course_id":item.id,
          "price_policy_id":item.choose_price_policy_id
        })
      })
